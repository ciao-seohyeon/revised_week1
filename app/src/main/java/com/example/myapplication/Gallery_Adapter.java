...
//        galley_item item = item_list.get(position);
        String currentFile = item_list.get(position);
//        String curPath = currentFile.getPath();

        ImageView imageView = holder.icon;

        Glide.with(mContext).load(currentFile).into(imageView);
...
//        holder.icon.setImageResource(item.getIcon());
    ...
    Gallery_Adapter(ArrayList<String> list, Context mContext) {
        item_list = list;
